...
@section('content')
    <div class="container-fluid h-100">
        <div class="row justify-content-center h-100">
                ...
                <div class="card mb-sm-3 mb-md-0 contacts_card">
                    ...
                    <div class="card-body contacts_body">
                        <ul class="contacts" id="users">
                            <li class="active" id="generals" onclick="generalchanel()">
                                <div class="d-flex bd-highlight">
                                    <div class="img_cont">
                                        <img src="/img/general.png" class="rounded-circle user_img">
            ...
            <div id="channels" class="col-md-8 col-xl-6 chat">
                <div class="card">
                    <div class="card-header msg_head">
                        <div class="d-flex bd-highlight">
                            <div class="img_cont">
                                <img id="channel_img" src="/img/general.png"
                                     class="rounded-circle user_img" alt="Không tìm thấy ảnh!">
                                <span class="online_icon"></span>
                            </div>
                            <div class="user_info">
                                <span id="channel_name">Kênh chat chung</span>
                            </div>
                        </div>
                        <span id="action_menu_btn" class="hidden"><i class="fas fa-ellipsis-v"></i></span>
                        <div class="action_menu">
                            <ul>
                                <li><i class="fas fa-user-circle"></i> Xem hồ sơ</li>
                                <li><i class="fas fa-users"></i> Thêm vào danh sách bạn</li>
                                <li><i class="fas fa-plus"></i> Thêm vào nhóm</li>
                                <li><i class="fas fa-ban"></i> Chặn người dùng</li>
                            </ul>
                        </div>
                    </div>
...
{{--                                <a><i class="far fa-hand-paper" style="color: #FFD43B; font-size: 35px;"></i></a>--}}
{{--                            </div>--}}
{{--                            <div class="input-group col-10">--}}
{{--                                <input id="message" type="text" class="form-control type_msg white-text"--}}
{{--                                       placeholder="Nhập tin nhắn...">--}}
{{--                            </div>--}}
{{--                            <div class="input-group col-1">--}}
{{--                                <button id="sentMessage" type="submit" class="btn btn-primary">Gửi</button>--}}
{{--                            </div>--}}
                    <div class="card-footer">
                        <form onsubmit="return false">
                            <div class="input-group">
                                <div class="input-group-append">
                                    <button id="sendGreet" type="submit" class="input-group-text attach_btn"><i class="far fa-hand-paper" style="font-size: 25px;"></i></button>
                                </div>
                                <input id="message" type="text" class="form-control type_msg" placeholder="Nhập tin nhắn...">
                                <input id="sender_id" type="hidden">
                                <input id="time" type="hidden">
                                <div class="input-group-append">
                                    <button id="sentMessage" type="submit" class="input-group-text send_btn"><i class="fas fa-location-arrow"></i></button>
                                </div>
                            </div>
                        </form>
                    </div>
...
@push('scripts')
    ...
    <script type="module">
        document.addEventListener("DOMContentLoaded", (event) => {
            window.axios.get('/list/message').then(response => {
                for (let i = 0; i < response.data.length; i++) {
                    const messagesElement = document.getElementById('messages');
                    const messageElement = document.createElement('li');
                    let html = '';
                    const date = new Date(response.data[i].created_at).toLocaleDateString('en-CA', {
                        day: "numeric",
                        month: "numeric",
                    });

                    const time = new Date(response.data[i].created_at).toLocaleTimeString('en-CA', {
                        hour12: false,
                        hour: "numeric",
                        minute: "numeric",
                    });
                    ...
                    messageElement.classList.add('list-unstyled');
                    if (e.sender_id == '{{ Auth::user()->id }}') {
                        html = '<div class="d-inline-flex justify-content-end mb-4">'
                            + '<div class="img_cont_msg">'
                            + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg">'
                            + '</div>'
                            + '<div class="msg_cotainer">'
                        ...
                        html = '<div class="d-inline-flex justify-content-start mb-4">'
                            + '<div class="img_cont_msg">'
                            + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg">'
                            + '</div>'
                            + '<div class="msg_cotainer">'
    ...
    <script type="text/javascript">
        function generalchanel() {
            const avtiveElement = document.getElementsByClassName('active');
            for (let i = 0; i < avtiveElement.length; i++) {
                avtiveElement[i].classList.remove('active');
            }
            const channel_name = document.getElementById('channel_name');
            const channel_img = document.getElementById('channel_img');
            const action_menu_btn = document.getElementById('action_menu_btn');
            const sender_id = document.getElementById('sender_id');
            channel_name.innerText = 'Kênh Chat Chung';
            channel_img.src = '/img/general.png';
            action_menu_btn.classList.add('hidden');
            sender_id.value = '';
        }
    </script>
            ...
            .here(
                (users) => {
                    users.forEach((user) => {
                        ...
                        userElement.setAttribute('onclick', `chatUser("{{ Auth::user()->id }}","${user.id}")`);
                        userElement.innerHTML = '<div class="d-flex bd-highlight">'
                            + '<div class="img_cont">'
                            + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img" alt="">'
                            + '<span class="online_icon"></span>'
                            + '</div>'
                            + '<div class="user_info">'
                            + '<span>' + user.name + '</span>'
                    ...
                    {{--document.getElementById({{ Auth::user()->id }}).innerHTML += '<div class="d-flex bd-highlight">'--}}
                    {{--+ '<div class="img_cont">'--}}
            ...
            .listen('SentMessage', (e) => {
                ...
                if (e.user.id == '{{ Auth::user()->id }}') {
                    ...
                    html =
                        ...
                        + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg">'
                        + '</div>'
                        + '<div class="msg_cotainer_send bd-highlight">'
                        + e.message
                        + '</div>'
                        + '<div class="msg_time_send">'
                        + currentTime
    ...
    <script type="module">
        ...
        // let now = new Date('YYYY-MM-DD HH:mm:ss').toLocaleString('en-US', { timeZone: 'Asia/Ho_Chi_Minh' })
        // let str = now.getUTCFullYear().toString() + "-" +
        ...
        sentMessage.addEventListener('click', (e) => {
            ...
            if (document.getElementById('sender_id').value === '') {
                window.axios.post('/chats/message', {
                    message: messagesElement.value,
                    time: currentDateTime,
                })
            } else {
                window.axios.post('/chats/private/message', {
                    receiver_id: document.getElementsByClassName('active').item(0).id,
                    message: messagesElement.value,
                    time: currentDateTime,
                })
            }
    ...
    <script>
        function chatUser(id, sender_id) {
            const avtiveElement = document.getElementsByClassName('active');
            for (let i = 0; i < avtiveElement.length; i++) {
                avtiveElement[i].classList.remove('active');
            }
            const userElement = document.getElementById(sender_id);
            userElement.classList.add('active');
            window.axios.get(`/chats/user/${id}/${sender_id}`).then(function(response) {
                const channel_name = document.getElementById('channel_name');
                const channel_img = document.getElementById('channel_img');
                const action_menu_btn = document.getElementById('action_menu_btn');
                document.getElementById('sender_id').value = response.data.user_receive.id;
                channel_name.innerText = response.data.user_receive.name;
                channel_img.src = 'https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg';
                action_menu_btn.classList.remove('hidden');
                });
            }
    ...
    <script type="module">
        const sender_id = document.getElementById('sender_id');
        Echo.private("chats.private.{{ Auth::user()->id }}").listen('SentPrivateMessage', (e) => {
            const messagesElement = document.getElementById('messages');
            const currentTime = new Date().toLocaleTimeString('en-US', {
                hour12: false,
                hour: "numeric",
            ...
            html = '<div class="d-flex align-items-start flex-column bd-highlight mb-4">'
                + '<div class="msg_cotainer bd-highlight">'
                + e.message
                + '</div>'
                + '<div class="msg_time">'
                + currentTime
