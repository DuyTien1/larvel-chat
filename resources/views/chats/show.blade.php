@extends('layouts.app')

@push('style')
    <style>
        body,html{
            height: 100%;
            margin: 0;
            background: #7F7FD5;
            background: -webkit-linear-gradient(to right, #91EAE4, #86A8E7, #7F7FD5);
            background: linear-gradient(to right, #91EAE4, #86A8E7, #7F7FD5);
        }

        .chat{
            margin-top: auto;
            margin-bottom: auto;
        }
        .card{
            height: 500px;
            border-radius: 15px !important;
            background-color: rgba(0,0,0,0.4) !important;
        }
        .contacts_body{
            padding:  0.75rem 0 !important;
            overflow-y: auto;
            white-space: nowrap;
        }
        .msg_card_body{
            overflow-y: auto;
        }
        .card-header{
            border-radius: 15px 15px 0 0 !important;
            border-bottom: 0 !important;
        }
        .card-footer{
            border-radius: 0 0 15px 15px !important;
            border-top: 0 !important;
        }
        .container{
            align-content: center;
        }
        .search{
            border-radius: 15px 0 0 15px !important;
            background-color: rgba(0,0,0,0.3) !important;
            border:0 !important;
            color:white !important;
        }
        .search:focus{
            box-shadow:none !important;
            outline:0px !important;
        }
        .type_msg{
            background-color: rgba(0,0,0,0.3) !important;
            border:0 !important;
            color:white !important;
            height: 60px !important;
            overflow-y: auto;
        }
        .type_msg:focus{
            box-shadow:none !important;
            outline:0px !important;
        }
        .attach_btn{
            border-radius: 15px 0 0 15px !important;
            background-color: rgba(0,0,0,0.3) !important;
            border:0 !important;
            color: white !important;
            cursor: pointer;
        }
        .send_btn{
            border-radius: 0 15px 15px 0 !important;
            background-color: rgba(0,0,0,0.3) !important;
            border:0 !important;
            color: white !important;
            cursor: pointer;
        }
        .search_btn{
            border-radius: 0 15px 15px 0 !important;
            background-color: rgba(0,0,0,0.3) !important;
            border:0 !important;
            color: white !important;
            cursor: pointer;
        }
        .contacts{
            list-style: none;
            padding: 0;
        }
        .contacts li{
            width: 100% !important;
            padding: 5px 10px;
            margin-bottom: 15px !important;
            cursor: pointer;
        }
        .active{
            background-color: rgba(0,0,0,0.3);
        }
        .user_img{
            height: 70px;
            width: 70px;
            border:1.5px solid #f5f6fa;

        }
        .user_img_msg{
            height: 40px;
            width: 40px;
            border:1.5px solid #f5f6fa;

        }
        .img_cont{
            position: relative;
            height: 70px;
            width: 70px;
        }
        .img_cont_msg{
            height: 40px;
            width: 40px;
            cursor: pointer;
        }
        .online_icon{
            position: absolute;
            height: 15px;
            width:15px;
            background-color: #4cd137;
            border-radius: 50%;
            bottom: 0.2em;
            right: 0.4em;
            border:1.5px solid white;
        }
        .offline{
            background-color: #c23616 !important;
        }
        .user_info{
            margin-top: auto;
            margin-bottom: auto;
            margin-left: 15px;
        }
        .user_info span{
            font-size: 20px;
            color: white;
        }
        .user_info p{
            font-size: 10px;
            color: rgba(255,255,255,0.6);
        }
        .video_cam{
            margin-left: 50px;
            margin-top: 5px;
        }
        .video_cam span{
            color: white;
            font-size: 20px;
            cursor: pointer;
            margin-right: 20px;
        }
        .msg_cotainer{
            margin-top: auto;
            margin-bottom: auto;
            margin-left: 10px;
            border-radius: 25px;
            background-color: #3EB489;
            padding: 10px;
            position: relative;
            cursor: pointer;
        }
        .msg_cotainer_send{
            margin-top: auto;
            margin-bottom: auto;
            margin-right: 10px;
            border-radius: 25px;
            background-color: #78e08f;
            padding: 10px;
            position: relative;
        }
        .msg_time{
            position: absolute;
            left: 10px;
            bottom: -15px;
            color: rgba(255,255,255,0.5);
            font-size: 10px;
        }
        .msg_time_send{
            position: absolute;
            right: 10px;
            bottom: -15px;
            color: rgba(255,255,255,0.5);
            font-size: 10px;
        }
        .msg_name{
            /*position: absolute;*/
            color: rgb(255, 157, 87);
            left: 15px;
            bottom: 27px;
            font-size: 11px;
        }
        .msg_name_send{
            /*position: absolute;*/
            color: rgb(255, 157, 87);
            right: 15px;
            bottom: 27px;
            font-size: 11px;
        }
        .msg_head{
            position: relative;
        }
        #action_menu_btn{
            position: absolute;
            right: 10px;
            top: 10px;
            color: white;
            cursor: pointer;
            font-size: 20px;
        }
        .action_menu{
            z-index: 1;
            position: absolute;
            padding: 15px 0;
            background-color: rgba(0,0,0,0.5);
            color: white;
            border-radius: 15px;
            top: 30px;
            right: 15px;
            display: none;
        }
        .action_menu ul{
            list-style: none;
            padding: 0;
            margin: 0;
        }
        .action_menu ul li{
            width: 100%;
            padding: 10px 15px;
            margin-bottom: 5px;
        }
        .action_menu ul li i{
            padding-right: 10px;

        }
        .action_menu ul li:hover{
            cursor: pointer;
            background-color: rgba(0,0,0,0.2);
        }
        @media(max-width: 576px){
            .contacts_card{
                margin-bottom: 15px !important;
            }
        }
        .user_select{
            user-select: none !important;
        }
    </style>
@endpush

@section('content')
    <div class="container-fluid h-100">
        <div class="row justify-content-center h-100">
            <div class="col-md-4 col-xl-3 chat">
                <div class="card mb-sm-3 mb-md-0 contacts_card">
                    <div class="card-header">
                        <div class="input-group">
                            <input type="text" placeholder="Tìm kiếm..." name="" class="form-control search">
                            <div class="input-group-prepend">
                                <span class="input-group-text search_btn"><i class="fas fa-search"></i></span>
                            </div>
                        </div>
                    </div>
                    <div class="card-body contacts_body">
                        <ul class="contacts user_select" id="users">
                            <li class="active" id="generals" onclick="generalchanel()">
                                <div class="d-flex bd-highlight">
                                    <div class="img_cont">
                                        <img src="/img/general.png" class="rounded-circle user_img" alt="">
                                        <span class="online_icon"></span>
                                    </div>
                                    <div class="user_info">
                                        <span>Kênh chat chung</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div class="card-footer"></div>
                </div>
            </div>
            <div id="channels" class="col-md-8 col-xl-6 chat">
                <div class="card">
                    <div class="card-header msg_head">
                        <div class="d-flex bd-highlight">
                            <div class="img_cont">
                                <img id="channel_img" src="/img/general.png"
                                     class="rounded-circle user_img" alt="Không tìm thấy ảnh!">
                                <span class="online_icon"></span>
                            </div>
                            <div class="user_info">
                                <span id="channel_name" class="user_select">Kênh chat chung</span>
                            </div>
                        </div>
                        <span id="action_menu_btn" class="hidden"><i class="fas fa-ellipsis-v"></i></span>
                        <div class="action_menu">
                            <ul>
                                <li><i class="fas fa-user-circle"></i> Xem hồ sơ</li>
                                <li><i class="fas fa-users"></i> Thêm vào danh sách bạn</li>
                                <li><i class="fas fa-plus"></i> Thêm vào nhóm</li>
                                <li><i class="fas fa-ban"></i> Chặn người dùng</li>
                            </ul>
                        </div>
                    </div>
                    <div class="card-body msg_card_body">
                        <ul id="messages">
{{--                            <li class="list-unstyled">--}}
{{--                                <div class="d-flex justify-content-start mb-4">--}}
{{--                                    <div class="img_cont_msg">--}}
{{--                                        <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg">--}}
{{--                                    </div>--}}
{{--                                    <div class="msg_cotainer">--}}
{{--                                        Hi, how are you samim?--}}
{{--                                        <span class="msg_time">8:40 AM, Today</span>--}}
{{--                                    </div>--}}
{{--                                </div>--}}
{{--                            </li>--}}
{{--                            <li class="list-unstyled">--}}
{{--                                <div class="d-flex justify-content-end mb-4">--}}
{{--                                    <div class="msg_cotainer_send">--}}
{{--                                        Ok, thank you have a good day--}}
{{--                                        <span class="msg_time_send">9:10 AM, Today</span>--}}
{{--                                    </div>--}}
{{--                                    <div class="img_cont_msg">--}}
{{--                                        <img--}}
{{--                                            src="data:image/jpeg;base64,"--}}
{{--                                            class="rounded-circle user_img_msg">--}}
{{--                                    </div>--}}
{{--                                </div>--}}
{{--                            </li>--}}
                        </ul>
                    </div>
{{--                            <div class="input-group col-1">--}}
{{--                                <a><i class="far fa-hand-paper" style="color: #FFD43B; font-size: 35px;"></i></a>--}}
{{--                            </div>--}}
{{--                            <div class="input-group col-10">--}}
{{--                                <input id="message" type="text" class="form-control type_msg white-text"--}}
{{--                                       placeholder="Nhập tin nhắn...">--}}
{{--                            </div>--}}
{{--                            <div class="input-group col-1">--}}
{{--                                <button id="sentMessage" type="submit" class="btn btn-primary">Gửi</button>--}}
{{--                            </div>--}}
                    <div class="card-footer">
                        <form onsubmit="return false">
                            <div class="input-group">
                                <div class="input-group-append">
                                    <button id="sendGreet" type="submit" class="input-group-text attach_btn"><i class="far fa-hand-paper" style="font-size: 25px;"></i></button>
                                </div>
                                <input id="message" type="text" class="form-control type_msg" placeholder="Nhập tin nhắn...">
                                <input id="sender_id" type="hidden">
                                <input id="time" type="hidden">
                                <div class="input-group-append">
                                    <button id="sentMessage" type="submit" class="input-group-text send_btn"><i class="fas fa-location-arrow"></i></button>
                                </div>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection

@push('scripts')
    <script type="text/javascript">
        $(document).ready(function () {
            $('#action_menu_btn').click(function () {
                $('.action_menu').toggle();
            });
        });
    </script>

    <script type="module">
        document.addEventListener("DOMContentLoaded", (event) => {
            window.axios.get('/list/message').then(response => {
                for (let i = 0; i < response.data.length; i++) {
                    if (response.data[i].channel === 'chats') {
                        const messagesElement = document.getElementById('messages');
                        const messageElement = document.createElement('li');
                        let html = '';
                        const date = new Date(response.data[i].created_at).toLocaleDateString('en-CA', {
                            day: "numeric",
                            month: "numeric",
                        });

                        const time = new Date(response.data[i].created_at).toLocaleTimeString('en-CA', {
                            hour12: false,
                            hour: "numeric",
                            minute: "numeric",
                        });

                        let dateTime = date + " " + time;
                        const e = response.data[i];
                        messageElement.classList.add('list-unstyled');
                        if (e.sender_id === '{{ Auth::user()->id }}') {
                            html = '<div class="d-inline-flex justify-content-end mb-4">'
                                + '<div class="img_cont_msg">'
                                + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg" alt="">'
                                + '</div>'
                                + '<div class="msg_cotainer">'
                                + '<div class="msg_name">'
                                + 'Nguyễn Duy Tiến'
                                + '</div>'
                                + e.message
                                + '<div class="msg_time">'
                                + dateTime
                                + '</div>'
                                + '</div>'
                                + '</div>';
                        } else {
                            html = '<div class="d-inline-flex justify-content-start mb-4">'
                                + '<div class="img_cont_msg">'
                                + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg" alt="">'
                                + '</div>'
                                + '<div class="msg_cotainer">'
                                + '<div class="msg_name">'
                                + 'Nguyễn Duy Tiến'
                                + '</div>'
                                + e.message
                                + '<div class="msg_time">'
                                + dateTime
                                + '</div>'
                                + '</div>'
                                + '</div>';
                        }
                        messageElement.innerHTML = html;
                        messagesElement.appendChild(messageElement);
                    }
                }
            })
        });
    </script>

    <script type="text/javascript">
        function generalchanel() {
            const activeElement = document.getElementsByClassName('active');
            for (let i = 0; i < activeElement.length; i++) {
                activeElement[i].classList.remove('active');
            }
            const generalsElement = document.getElementById('generals');
            generalsElement.classList.add('active');
            const channel_name = document.getElementById('channel_name');
            const channel_img = document.getElementById('channel_img');
            const action_menu_btn = document.getElementById('action_menu_btn');
            const sender_id = document.getElementById('sender_id');
            channel_name.innerText = 'Kênh Chat Chung';
            channel_img.src = '/img/general.png';
            action_menu_btn.classList.add('hidden');
            sender_id.value = '';
        }
    </script>

    <script type="module">
        const usersElement = document.getElementById('users');
        const messagesElement = document.getElementById('messages');

        const currentDate = new Date().toLocaleDateString('en-CA', {
            timeZone: 'Asia/Ho_Chi_Minh',
            day: "numeric",
            month: "numeric",
        });

        const currentTime = new Date().toLocaleTimeString('en-CA', {
            timeZone: 'Asia/Ho_Chi_Minh',
            hour12: false,
            hour: "numeric",
            minute: "numeric",
        });

        let currentDateTime = currentDate + " " + currentTime;

        Echo.join('chats')
            .here(
                (users) => {
                    users.forEach((user) => {
                        if (user.id == {{ Auth::user()->id }}) {
                            return;
                        }
                        const userElement = document.createElement('li');
                        userElement.setAttribute('id', user.id);
                        userElement.setAttribute('onclick', `chatUser("${user.id}")`);
                        userElement.innerHTML = '<div class="d-flex bd-highlight">'
                            + '<div class="img_cont">'
                            + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img" alt="">'
                            + '<span class="online_icon"></span>'
                            + '</div>'
                            + '<div class="user_info">'
                            + '<span>' + user.name + '</span>'
                            + '<p>' + 'Đang trực tuyến' + '</p>'
                            + '</div>'
                            + '</div>';
                        usersElement.appendChild(userElement);
                    });
                }
            )
            .joining(
                (user) => {
                    const userElement = document.createElement('li');
                    userElement.setAttribute('id', user.id);
                    userElement.setAttribute('onclick', `chatUser("${user.id}")`);
                    userElement.innerHTML = '<div class="d-flex bd-highlight">'
                        + '<div class="img_cont">'
                        + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img" alt="">'
                        + '<span class="online_icon"></span>'
                        + '</div>'
                        + '<div class="user_info">'
                        + '<span>' + user.name + '</span>'
                        + '<p>' + 'Đang trực tuyến' + '</p>'
                        + '</div>'
                        + '</div>';
                    usersElement.appendChild(userElement);
                }
            )
            .leaving(
                (user) => {
                    const userElement = document.getElementById(user.id);
                    usersElement.removeChild(userElement);
                }
            )
            .listen('SentMessage', (e) => {
                const messageElement = document.createElement('li');
                let html = '';
                messageElement.classList.add('list-unstyled');
                if (e.user.id == '{{ Auth::user()->id }}') {
                    html = '<div class="d-inline-flex justify-content-end mb-4">'
                        + '<div class="img_cont_msg">'
                        + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg" alt="">'
                        + '</div>'
                        + '<div class="msg_cotainer">'
                        + '<div class="msg_name">'
                        + 'Nguyễn Duy Tiến'
                        + '</div>'
                        + e.message
                        + '<div class="msg_time">'
                        + currentDateTime
                        + '</div>'
                        + '</div>'
                        + '</div>';
                } else {
                    html = '<div class="d-inline-flex justify-content-start mb-4">'
                        + '<div class="img_cont_msg">'
                        + '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg" alt="">'
                        + '</div>'
                        + '<div class="msg_cotainer">'
                        + '<div class="msg_name">'
                        + 'Nguyễn Duy Tiến'
                        + '</div>'
                        + e.message
                        + '<div class="msg_time">'
                        + currentDateTime
                        + '</div>'
                        + '</div>'
                        + '</div>';
                }
                messageElement.innerHTML = html;
                messagesElement.appendChild(messageElement);
            })
    </script>

    <script type="module">
        const messagesElement = document.getElementById('message');
        const sentMessage = document.getElementById('sentMessage');

        const currentDate = new Date().toLocaleDateString('en-CA', {
            timeZone: 'Asia/Ho_Chi_Minh',
            day: "numeric",
            month: "numeric",
            year: "numeric",
        });

        const currentTime = new Date().toLocaleTimeString('en-CA', {
            timeZone: 'Asia/Ho_Chi_Minh',
            hour12: false,
            hour: "numeric",
            minute: "numeric",
            second: "numeric"
        });

        let currentDateTime = currentDate + " " + currentTime;
        const receiver_id = document.getElementsByClassName('active').item(0).id;



        sentMessage.addEventListener('click', (e) => {
            e.preventDefault();
            if (messagesElement.value === '') {
                return;
            }
            if (document.getElementById('sender_id').value === '') {
                window.axios.post('/chats/message', {
                    message: messagesElement.value,
                    time: currentDateTime,
                })
            } else {
                window.axios.post(`/chats/private/${receiver_id}`, {
                    message: messagesElement.value,
                    time: currentDateTime,
                })
            }
            messagesElement.value = '';
        })

    </script>

    <script>
        function chatUser(receiver) {
            console.log(`chats/user/${receiver}`);
            const activeElement = document.getElementsByClassName('active');
            for (let i = 0; i < activeElement.length; i++) {
                activeElement[i].classList.remove('active');
            }
            const userElement = document.getElementById(receiver);
            userElement.classList.add('active');
            window.axios.get(`chats/user/${receiver}`).then(function(response) {
                const channel_name = document.getElementById('channel_name');
                const channel_img = document.getElementById('channel_img');
                const action_menu_btn = document.getElementById('action_menu_btn');
                document.getElementById('sender_id').value = response.data.receiver.id;
                channel_name.innerText = response.data.receiver.name;
                channel_img.src = 'https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg';
                action_menu_btn.classList.remove('hidden');
                });
            }
    </script>

@endpush
